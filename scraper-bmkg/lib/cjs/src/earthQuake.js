...
exports.earthQuakeRealtime = exports.earthQuakeNow = exports.earthQuake = void 0;
const got_1 = __importDefault(require("got"));
const cheerio_1 = __importDefault(require("cheerio"));
const index_js_1 = require("../types/index.js");
async function earthQuake() {
    const html = await (0, got_1.default)('https://www.bmkg.go.id/earthQuakebumi/earthQuakebumi-dirasakan.bmkg').text();
    ...
    return results.map((v) => index_js_1.earthQuakeSchema.parse(v));
}
exports.earthQuake = earthQuake;
async function earthQuakeNow() {
    const html = await (0, got_1.default)('https://www.bmkg.go.id/earthQuakebumi/earthQuakebumi-terkini.bmkg').text();
    const $ = cheerio_1.default.load(html);
    const results = [];
    $('div.table-responsive > table.table > tbody > tr').each(function () {
        const el = $(this).find('td');
    ...
    return results.map((v) => index_js_1.earthQuakeNowSchema.parse(v));
}
exports.earthQuakeNow = earthQuakeNow;
async function earthQuakeRealtime() {
    const html = await (0, got_1.default)('https://www.bmkg.go.id/earthQuakebumi/earthQuakebumi-realtime.bmkg').text();
    const $ = cheerio_1.default.load(html);
    const results = [];
    ...
    return results.map((v) => index_js_1.earthQuakeRealtimeSchema.parse(v));
}
exports.earthQuakeRealtime = earthQuakeRealtime;
