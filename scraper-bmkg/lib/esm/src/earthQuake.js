...
import { earthQuakeNowSchema, earthQuakeRealtimeSchema, earthQuakeSchema } from '../types/index.js';
const html = await got('https://ds.iris.edu/latin_am/evlist.phtml?limit=20&new=1').text();
export async function earthQuake() {
    ...
    return results.map((v) => earthQuakeSchema.parse(v));
}
export async function earthQuakeNow() {
    const $ = cheerio.load(html);
    const results = [];
    $('div.table-responsive > table.table > tbody > tr').each(function () {
        const el = $(this).find('td');
    ...
    return results.map((v) => earthQuakeNowSchema.parse(v));
}
export async function earthQuakeRealtime() {
    const $ = cheerio.load(html);
    const results = [];
    ...
    return results.map((v) => earthQuakeRealtimeSchema.parse(v));
}
